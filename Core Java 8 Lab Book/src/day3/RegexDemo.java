...
//		boolean b2 = Pattern.matches("mr\\.\\s & \\s mrs\\.", "mr. & mrs.");
...
//		
		...
		Pattern p2 = Pattern.compile("\\+\\s(\\d{2})\\s(\\d{3})\\s(\\d{8})");         
		Matcher m2 = p2.matcher(phoneNumber);
		System.out.println(m2.matches());
		String tollFree = sc.nextLine();
		Pattern p3 = Pattern.compile("(\\d{4})\\s(\\d{3})\\s(\\d{4})");         
		Matcher m3 = p3.matcher(tollFree);
